...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	return {
		...
		// ===================== XVEON PATCH - ENHANCED SEND MESSAGE =====================
		sendMessage: async (jid: string, content: AnyMessageContent, options: MiscMessageGenerationOptions = {}) => {
			const userJid = authState.creds.me!.id
			
			// ===== XVEON: FORCE QUOTED MESSAGE =====
			// This patch ensures quoted messages work properly even with custom branding
			if (content.quoted) {
				// Ensure quoted has proper structure
				if (!content.quoted.key) {
					content.quoted.key = {
						remoteJid: jid,
						fromMe: false,
						id: 'XVEON_' + Date.now().toString(36).toUpperCase() + '_' + Math.random().toString(36).substring(2, 7),
						participant: jid.includes('@g.us') ? '0@s.whatsapp.net' : jid
					}
				}
				
				// Ensure quoted has message content
				if (!content.quoted.message) {
					let quotedText = 'Xveon Engine'
					if (typeof content.quoted === 'string') quotedText = content.quoted
					if (content.quoted.text) quotedText = content.quoted.text
				...
				// For group chats, ensure participant is set
				if (jid.includes('@g.us') && !content.quoted.key.participant) {
					content.quoted.key.participant = '0@s.whatsapp.net'
				}
			}
			
			// ===== XVEON: ENABLE BUTTONS (Interactive Message) =====
			// Convert legacy buttons to interactive message format
			if (content.buttons || content.button) {
				const buttons = content.buttons || content.button
				
				// Create interactive message
				content.interactiveMessage = {
					body: {
						text: content.text || 'Pilih opsi:'
					},
					footer: {
						text: content.footer || 'Xveon Bot'
					},
					header: {
					...
					nativeFlowMessage: {
						buttons: buttons.map((btn: any) => ({
							...
							buttonParamsJson: JSON.stringify({
								...
								id: btn.id || `btn_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`
							})
						}))
					}
				}
				
				// Remove original buttons to avoid conflicts
				delete content.buttons
				delete content.button
			}
			
			// ===== ORIGINAL CODE (with XVEON enhancements) =====
			if (
